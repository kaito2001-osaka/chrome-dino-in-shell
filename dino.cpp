...
// Crouched dinosaur. Only DUCK_H rows tall, which is what lets it pass under a
// pterodactyl that a standing dinosaur walks into.
const std::string DUCK_AA[] = {
    "   ____ ",
    "__/o_|_|"
};

// Small cactus. Each row is padded to CACTUS_SMALL_W characters.
const std::string CACTUS_SMALL_AA[] = {
    " | ",
    "\\|/",
    " | "
};
...
// Two and three small cacti drawn as one obstacle, packed with no gap: a wider
// cluster needs a longer airborne stretch to clear than the jump provides.
const std::string CLUSTER2_AA[] = {
    " |  | ",
    "\\|/\\|/",
    " |  | "
};

const std::string CLUSTER3_AA[] = {
    " |  |  | ",
    "\\|/\\|/\\|/",
    " |  |  | "
};

// Pterodactyl, flown at head height so it has to be ducked, not jumped.
const std::string PTERO_AA[] = {
    "\\___/",
    " \\o/ ",
    "  v  "
};
...
// Every sprite, paired with the constants it has to match. The row count comes
// from the array bound rather than from the *_H constant: passing the constant
// on both sides would make the row check compare a number with itself.
namespace {

struct ArtSpec {
    const std::string* rows;
    size_t actual_rows; // From the array bound, not restated by hand
    int declared_w;
    int declared_h;
    const char* name;
};
...
template <size_t N>
ArtSpec Spec(const std::string (&rows)[N], int width, int height, const char* name) {
    ArtSpec spec = {rows, N, width, height, name};
    return spec;
}

} // namespace

// Game will not run if the art's dimensions do not match the dimensions
// declared for it in dino.h. Adding a sprite means adding one line here.
bool ValidateArt() {
    const ArtSpec specs[] = {
        Spec(DINO_AA,         DINO_W,         DINO_H,         "DINO_AA"),
        Spec(DUCK_AA,         DUCK_W,         DUCK_H,         "DUCK_AA"),
        Spec(CACTUS_SMALL_AA, CACTUS_SMALL_W, CACTUS_SMALL_H, "CACTUS_SMALL_AA"),
        Spec(CACTUS_AA,       CACTUS_W,       CACTUS_H,       "CACTUS_AA"),
        Spec(CLUSTER2_AA,     CLUSTER2_W,     CLUSTER2_H,     "CLUSTER2_AA"),
        Spec(CLUSTER3_AA,     CLUSTER3_W,     CLUSTER3_H,     "CLUSTER3_AA"),
        Spec(PTERO_AA,        PTERO_W,        PTERO_H,        "PTERO_AA"),
    };

    for (size_t i = 0; i < sizeof(specs) / sizeof(specs[0]); ++i) {
        const ArtSpec& art = specs[i];

        if (art.actual_rows != static_cast<size_t>(art.declared_h)) {
            std::cerr << "dino: " << art.name << " has " << art.actual_rows
                      << " rows, but its height constant says " << art.declared_h
                      << "\n";
            return false;
        }

        for (int row = 0; row < art.declared_h; ++row) {
            if (art.rows[row].length() != static_cast<size_t>(art.declared_w)) {
                std::cerr << "dino: " << art.name << " row " << row << " is "
                          << art.rows[row].length()
                          << " characters, but its width constant says "
                          << art.declared_w << "\n";
                return false;
            }
        }
...
const std::string* Game::DinoArt() const {
...
void Game::CheckCollision() {
    const std::string* dino_art = DinoArt();
